...
class AppSettings(BaseSettings):
    ...
    azure_env_name: str = Field("", env="AZURE_ENV_NAME")
    azure_storage_account_name: str = Field("", env="AZURE_STORAGE_ACCOUNT_NAME")
    azure_search_service_name: str = Field("", env="AZURE_SEARCH_SERVICE_NAME")
    azure_app_service_name: str = Field("", env="AZURE_APP_SERVICE_NAME")
    azure_resource_group: str = Field("", env="AZURE_RESOURCE_GROUP")  # optional, supplied by azd
    ...
    azure_sql_database: str = Field("arclog", env="AZURE_SQL_DATABASE_NAME")
    use_aad: bool = Field(..., env="USE_AAD")

    # Other
    client_public_ip: Optional[str] = Field(None, env="CLIENT_PUBLIC_IP")
    ...
    class Config:
        ...
        env_priority = True
        extra = "ignore"  # ignore unexpected variables like CLIENT_PUBLIC_IP, AZURE_RESOURCE_GROUP
